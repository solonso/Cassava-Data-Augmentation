...
class ImgAug():
    ...
    def APP2(self):
        ...
        def rand_shear():
            self.append += 1
            sh_angle = np.radians((30-(0))*np.random.random_sample()-0)
            img = cv2.imread(self.img_path)
            #img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
            plt.axis("off")
            rows, cols, dim = img.shape
            M = np.float32([[1, sh_angle, 0],
            [sh_angle, 1, 0],
            [0,0,1]])
            sheared_img = img.copy()
            sheared_img = cv2.warpPerspective(img, M, (int(cols*1.5), int(rows*1.5)))
            #plt.imshow(sheared_img)
            #plt.show()
            cv2.imwrite(os.path.join(self.root_path, self.img_name[:-4]+'_'+str(self.append))+'.jpg', sheared_img)
        rand_shear()
